"""
Script huấn luyện model Qwen2.5-0.5B-Instruct với LoRA - Phiên bản v2
Cải thiện hyperparameters để tránh overfitting và tăng generalization.
"""
...
# ============================================================================
# CONFIGURATION - IMPROVED FOR BETTER GENERALIZATION
# ============================================================================
...
# Sử dụng merged data (nếu có), fallback về original
TRAIN_FILE = DATA_DIR / "slm_train_merged.jsonl"
VAL_FILE = DATA_DIR / "slm_val_merged.jsonl"

# Fallback nếu chưa merge
if not TRAIN_FILE.exists():
    TRAIN_FILE = DATA_DIR / "slm_train.jsonl"
    VAL_FILE = DATA_DIR / "slm_val.jsonl"

OUTPUT_DIR = BASE_DIR / "models" / "qwen2.5-0.5b-med-slm-lora-v2"
...
# ============================================================================
LORA_R = 16  # Tăng từ 8 lên 16 để model có nhiều capacity hơn
LORA_ALPHA = 32  # Tăng tương ứng (thường = 2 * r)
LORA_DROPOUT = 0.1  # Tăng từ 0.05 lên 0.1 để tránh overfitting
LORA_TARGET_MODULES = ["q_proj", "k_proj", "v_proj", "o_proj"]

# ============================================================================
# Training Configuration - IMPROVED
# ============================================================================
NUM_EPOCHS = 3  # Tăng lên 3 epochs để học kỹ hơn
BATCH_SIZE = 8
LEARNING_RATE = 2e-5  # Giảm từ 5e-5 xuống 2e-5 để học chậm và ổn định hơn
LOGGING_STEPS = 50
SAVE_STEPS = 500
EVAL_STEPS = 500
MAX_LENGTH = 256
WARMUP_RATIO = 0.1
WEIGHT_DECAY = 0.01  # Thêm weight decay để regularization
...
def apply_lora(model):
    """Áp dụng LoRA vào model với cấu hình cải thiện."""
    print("\nApplying LoRA configuration (improved)...")
    print(f"  r = {LORA_R}")
...
def evaluate_model(model, tokenizer, val_data: List[Dict], device: str) -> Dict:
    ...
    with torch.no_grad():
        for i, item in enumerate(val_data):
            ...
            if "TRUE" in generated_text or "ĐÚNG" in generated_text:
                pred_label = "TRUE"
            elif "FALSE" in generated_text or "SAI" in generated_text:
                pred_label = "FALSE"
            else:
                pred_label = generated_text[:10]
...
def train():
    ...
    print("=" * 60)
    print("TRAINING QWEN2.5-0.5B WITH LORA v2 (IMPROVED)")
    print("=" * 60)
    ...
    # Apply LoRA
    print("\n3. Applying LoRA (improved config)...")
    ...
    print("\n5. Setting up training (improved config)...")
    training_args = TrainingArguments(
        ...
        weight_decay=WEIGHT_DECAY,  # Thêm regularization
    ...
    accuracy_result = evaluate_model(model, tokenizer, val_data[:500], DEVICE)  # Sample 500 for speed
    ...
    # Summary
    print("\n" + "=" * 60)
    print("TRAINING SUMMARY (v2 - IMPROVED)")
    print("=" * 60)
