"""
Script test model Qwen LoRA trên tệp held-out từ Test_sample
(Dùng sau khi train với dữ liệu merged)
"""
...
TEST_FILE = BASE_DIR / "data" / "slm_test_sample_held_out.jsonl"  # File held-out
LORA_MODEL_DIR = BASE_DIR / "models" / "qwen2.5-0.5b-med-slm-lora-v2"  # Model v2
BASE_MODEL_NAME = "Qwen/Qwen2.5-0.5B-Instruct"
OUTPUT_FILE = BASE_DIR / "data" / "test_held_out_results.json"
...
def evaluate_on_held_out():
    """Đánh giá model trên held-out test set."""
    print("=" * 60)
    print("TESTING QWEN LORA V2 ON HELD-OUT TEST SET")
    print("=" * 60)
    
    # Check if test file exists
    if not TEST_FILE.exists():
        print(f"ERROR: Test file not found: {TEST_FILE}")
        print("Please run merge_datasets.py first to create the held-out test set.")
        return None
    ...
    for i, sample in enumerate(samples):
        # Format: {"input": "Nhận định sau đây...: <text>", "output": "TRUE/FALSE"}
        input_text = sample["input"]
        true_label = sample["output"].strip().upper()
        
        # Trích xuất phần text thực sự (bỏ prefix "Nhận định sau đây...")
        # Hoặc dùng luôn input_text nếu prompt đã được format
        if "Nhận định sau đây đúng hay sai về y khoa:" in input_text:
            text_only = input_text.replace("Nhận định sau đây đúng hay sai về y khoa:", "").strip()
        else:
            text_only = input_text
        
        pred_label, raw_output = predict_single(model, tokenizer, text_only)
